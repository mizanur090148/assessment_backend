...
export class ProductService {
  ...
  async create(createProductDto: CreateProductDto): Promise<any> {
    ...
    const existComplain = await this.chiefComplainModel.findOne({
      name,
      doctorId,
    });
    if (existComplain) {
      throw new HttpException('Complain already exist', HttpStatus.BAD_REQUEST);
    }

    try {
      return await this.chiefComplainModel.create(createChiefComplainDto);
    } catch (error) {
